(function () {
  ...
  const SUPABASE_URL =
    window.__SUPABASE_URL__ || 'https://upihgyqkcyqjqvclujpm.supabase.co';
  const SUPABASE_KEY =
    window.__SUPABASE_KEY__ || 'sb_publishable_ri5QOHd4SUrVkCG696PIWA_IINiPZ-y';
  ...
  function installPersistenceBridge() {
    ...
    async function pushContent(raw) {
      ...
      try {
        ...
        window.dispatchEvent(
          new CustomEvent('supabase-save-status', {
            detail: { ok: true, message: 'Salvo no Supabase' }
          })
        );
      } catch (err) {
        ...
        window.dispatchEvent(
          new CustomEvent('supabase-save-status', {
            detail: { ok: false, message: 'Erro ao salvar no Supabase' }
          })
        );
      }
    }
    ...
    Storage.prototype.setItem = function (key, value) {
      ...
      if (this === localStorage && key === STORAGE_KEY) {
        ...
        window.dispatchEvent(
          new CustomEvent('supabase-save-status', {
            detail: { ok: true, pending: true, message: 'Salvando...' }
          })
        );

        clearTimeout(saveTimer);
        saveTimer = setTimeout(function () {
          pushContent(lastPayload);
        }, 500);
      }
    };
        ...
        window.dispatchEvent(
          new CustomEvent('supabase-save-status', {
            detail: {
              ok: true,
              pending: true,
              message: 'Upload feito, gerando link...'
            }
          })
        );
  ...
  function waitForAppRender(timeoutMs) {
  ...
  function loadApp() {
    return new Promise(function (resolve, reject) {
      ...
      script.onerror = function () {
        reject(new Error('Falha ao carregar módulo principal: ' + MAIN_MODULE));
      };
  ...
  preloadContent()
    ...
    .then(function () {
      return waitForAppRender(5000);
    })
    .then(function () {
      hideLoadingScreen();
    })
    .catch(function (err) {
      console.error('[bootstrap] erro ao iniciar app', err);
      hideLoadingScreen();
    });
})();
